...
@if($pagination['pages'] > 1)
            <nav class="pagination {{ $pagination['pages'] > $dropdown_limit ? 'dropdown' : '' }}">
                ...
                @if($pagination['pages'] > $dropdown_limit && $pagination['page'] > 1)
                    <li class="pagination_first"><a href="@base('/blog')" title="@lang('first')">&laquo;</a></li>
                @endif
                @if($pagination['page'] == 2)
                    <li class="pagination_previous"><a href="@base('/blog')" title="@lang('previous')">&lt;</a></li>
                @endif
                @if($pagination['page'] > 2)
                    <li class="pagination_previous"><a href="@base('/blog/'.($pagination['page']-1))" title="@lang('previous')">&lt;</a></li>
                @endif
                ...
                @for($i = 1; $i <= $pagination['pages']; $i++)
                    ...
                    @else
                        <a href="@base('/blog'.($i == 1 ? '' : '/'.$i))" title="@lang('Page') {{ $i }} @lang('of') {{ $pagination['pages'] }}">{{ $i }}</a>
                    @endif
                    </li>
                @endfor
                @if($pagination['pages'] > $dropdown_limit)
                ...
                @if($pagination['page'] < $pagination['pages'])
                    <li class="pagination_next"><a href="@base('/blog/'.($pagination['page']+1))" title="@lang('next')">&gt;</a></li>
                @endif
                @if($pagination['pages'] > $dropdown_limit && $pagination['page'] < $pagination['pages'])
                    <li class="pagination_last"><a href="@base('/blog/'.$pagination['pages'])" title="@lang('last')">&raquo;</a></li>
                @endif
                </ul>
            </nav>
@endif
